...
<div class="navbar bg-base-100 lg:hidden sticky top-0 w-full z-50">
 ...
 <div  class="flex justify-between w-12 mx-auto pb-2">
  ...
  <div class="p-6 mx-4 my-6 rounded-lg border border-gray-200 shadow-md ">
    <a href="{{ url('category/'.$latest_post_item->kategori->slug.'/'.$latest_post_item->slug) }}">
        <h5 class="mb-2 text-2xl font-bold tracking-tig ">{{ $latest_post_item->name }}</h5>
    </a>
    <p class="mb-3 font-normal">Posted On : {{ $latest_post_item->created_at->format('d-m-Y') }}.</p>
    <p class="mb-3 font-normal">Posted By : {{ $latest_post_item->user->name }}.</p>
    <a href="{{ url('category/'.$latest_post_item->kategori->slug.'/'.$latest_post_item->slug) }}" class="inline-flex items-center py-2 px-3 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300">
        Read more
